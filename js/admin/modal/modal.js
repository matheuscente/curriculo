    ...
    <legend class="modal-project-label">Tecnologias</legend>
      ...
      return `
        <div class="modal-project-overlay" id="modalProjectOverlay">
          <div class="modal-project-container">
            <p style="text-align: center" class="modal-confirm-text">Aguardando dados...<p/>
            <p>
...
//abre o modal
 async openModal(target, action, parentElement) {
  ...
  async putItem(formData, target, tecnologiasSelecionadas) {
    let data = {};
          ...
          for (const field in formItems) {
            ...
            await api.putData(`https://cms-gwjr.onrender.com/api/v1/${target.id}/${target.value}`, data, {
  ...
  async postItem(formData, target, tecnologiasSelecionadas) {
          ...
          await api.postData(`https://cms-gwjr.onrender.com/api/v1/${target.id}`, data, {
              withCredentials: true
            })
